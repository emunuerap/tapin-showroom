export function WebSDKScene() {
  return (
    <div className="relative h-full min-h-[34rem] overflow-hidden rounded-[2rem] border border-white/10 bg-[#080808] p-5 shadow-[0_30px_90px_rgba(0,0,0,0.52)]">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_80%_22%,rgba(204,255,0,0.12),transparent_32%)]" />
      <div className="relative h-full rounded-[1.5rem] border border-white/10 bg-[#111]/78 p-5">
        <div className="flex items-center justify-between border-b border-white/10 pb-4">
          ...
          <div className="rounded-full border border-white/10 px-4 py-2 font-mono text-[9px] uppercase tracking-[0.2em] text-silver/52">
            menu / events / book
          </div>
        </div>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-[1fr_0.74fr]">
          <div className="space-y-4">
            <div className="h-40 rounded-2xl border border-white/10 bg-[linear-gradient(135deg,rgba(255,255,255,0.08),rgba(255,255,255,0.015))]" />
            <div className="grid grid-cols-3 gap-3">
              {[1, 2, 3].map((item) => (
                <div key={item} className="h-24 rounded-xl border border-white/8 bg-white/[0.025]" />
              ))}
            </div>
          </div>
